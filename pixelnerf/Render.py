...
def get_coarse_query_points(tn, tf, N_samples, device):
    ...
    pt_value_near = torch.linspace(tn, 1.0, N_samples // 2 + 1, device=device)[:-1]
    pt_value_near += k[:N_samples // 2]
    # 在深度大于1的部分进行线性视差采样
    pt_value_far = torch.linspace(1.0, tf, N_samples // 2 + 1, device=device)[:-1]
    pt_value_far += (torch.arange(N_samples // 2, device=device) / float(N_samples // 2)) * (tf - 1.0)
    # pt_value = torch.linspace(0.5, 1.0, N_samples + 1, device=device)[:-1]
    # pt_value += k
    pt_value = torch.cat((pt_value_near, pt_value_far))
