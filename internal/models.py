...
class MLP(nn.Module):
    ...
    net_width_viewdirs: int = 32  # The width of the second part of MLP.
    ...
    def compute_jacobian_determinant(self, x, warp_fn):
        """Compute the Jacobian determinant of the transformation."""
        # 使用自动微分计算Jacobian
        jacobian = torch.autograd.functional.jacobian(warp_fn, x)

        # 计算Jacobian矩阵的行列式，假设是3D空间
        jacobian_det = torch.linalg.det(jacobian)

        # 返回行列式的绝对值
        return jacobian_det.abs()
    ...
    def predict_density(self, means, stds, rand=False, no_warp=False):
        ...
        if self.warp_fn is not None and not no_warp:
            means, stds, det_jacobi = coord.track_linearize(self.warp_fn, means, stds)
            # contract [-2, 2] to [-1, 1]
            bound = 2
            means = means / bound
            stds = stds / bound
        ...
        det_jacobi = det_jacobi.mean(dim=-1)  # 对最后一维求均值
        raw_density = x[..., 0]  # Hardcoded to a single channel.
        raw_density *= det_jacobi
...
def render_image(model,
  ...
  """
    ...
    # 新的
    # batch = {k: v.reshape((num_rays, -1)) if k != 'pose' and v is not None else v for k, v in batch.items()}
