...
def track_linearize(fn, mean, std):
  ...
  """
    ...
    mean, jacobi = fn(mean)  # 获取z和jacobi
    det_jacobi = torch.abs(torch.linalg.det(jacobi))  # 计算Jacobian行列式的绝对值

    det_jacobi = det_jacobi.reshape(*pre_shape)
    mean = mean.reshape(*pre_shape, 3)
    std = std.reshape(*pre_shape)
    return mean, std, det_jacobi
