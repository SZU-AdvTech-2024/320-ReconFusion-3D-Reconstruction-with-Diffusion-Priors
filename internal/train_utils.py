...
def compute_sample_loss(rendered_image, generated_image, t):
    device = rendered_image.device
    # 计算 L1 损失
    l1_loss = torch.abs(rendered_image - generated_image).mean()
    # 计算 LPIPS 损失
    loss_fn = lpips.LPIPS(net='alex')
    loss_fn = loss_fn.to(device)
    lpips_loss = loss_fn(rendered_image, generated_image)
    # 计算噪声级别依赖的加权函数
    w_t = torch.exp(-t)  # 或者根据实际情况选择其他加权函数

    # 计算总损失
    total_loss = w_t * (l1_loss + lpips_loss)
