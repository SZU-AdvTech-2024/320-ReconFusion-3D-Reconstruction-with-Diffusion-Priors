...
class Dataset(torch.utils.data.Dataset):
  ...
  """
    ...
    """
        ...
        # Add poses to the batch
        batch['pose'] = self.camtoworlds[cam_idx]
...
class LLFF(Dataset):
    ...
    def choose_views(self, poses, num_samples, num_views, object_center=np.zeros(3)):
        """
        Select train/test views with uniform distribution around the object.
        Args:
            poses: (N, 4, 4) array of camera poses.
            num_samples: Number of random splits to evaluate.
            num_views: Number of views in each split.
            object_center: Center of the object of interest.
        Returns:
            Selected view indices.
        """
        camera_centers = poses[:, :3, 3]  # Extract camera centers
        best_split = None
        min_loss = float('inf')

        for _ in range(num_samples):
            split_indices = np.random.choice(len(poses), num_views, replace=False)
            split_centers = camera_centers[split_indices]

            # Compute heuristic loss: maximize uniformity
            distances = np.linalg.norm(split_centers - object_center, axis=-1)
            loss = np.std(distances)  # Standard deviation as a measure of uniformity
    ...
    def _load_renderings(self, config):
        ...
        if not os.path.exists(colmap_dir):
            raise ValueError(f'COLMAP directory not found: {colmap_dir}')
        ...
        if config.forward_facing:
            ...
            if config.render_spline_keyframes is not None:
                ...
                save_path = os.path.join(self.data_dir, "render_poses.npy")
                np.save(save_path, self.render_poses)
                print(f"Render poses saved to {save_path}")
        # Select the split.
        ...
        # 选择均匀分布的三视图作为训练集
        train_indices = self.choose_views(poses, num_samples=1, num_views=3)
        train_indices = train_indices[:3]  # 只保留前 3 个视图的索引
        # 剩余视图作为测试集
